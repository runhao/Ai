...
def equalization():
    ...
    for x in range(len(dirs)):
        ...
        for img_file in imgs[x]:  # 遍历
            ...
            name = img_file[0:pos]  # 取出名称部分
            suffix = img_file[pos:]  # 取出后缀名
            original = cv.imread(img_full_path)
            # BRG空间转换为YUV空间
            # YUV：亮度，色度，饱和度，其中Y通道为亮度通道
            yuv = cv.cvtColor(original, cv.COLOR_BGR2YUV)
            yuv[..., 0] = cv.equalizeHist(yuv[..., 0])  # 取出亮度通道，均衡化并赋回原图像
            equalized_color = cv.cvtColor(yuv, cv.COLOR_YUV2BGR)
            img_new_name = "%s_equal%s" % (name, suffix)
            cv.imwrite(os.path.join(sub_dir_path[x], img_new_name), equalized_color)
            print("equal:", os.path.join(dd, "data", img_new_name))
        os.chdir(os.path.join(dd, "data"))  # 返回上级目录
